...
export class MapComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.layers = this.initialCoordinates.map(value => new Marker([value.latitude, value.longitude], this.markerOptions));
  }
  ...
  handleClick(event: LeafletMouseEvent) {
    ...
    this.layers = [];
    this.layers.push(new Marker([latitude, longitude], this.markerOptions));
